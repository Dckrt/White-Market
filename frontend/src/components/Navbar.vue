  ...
  <nav class="nav">
    <div class="nav__inner">
      ...
      <div class="nav__actions" v-if="auth.user">
        ...
        <!-- Notifications -->
        <!-- FIX: use ref + document mousedown instead of v-click-outside + @click.stop -->
        <div class="nav__icon-btn nav__notif-wrap" ref="notifRef" title="Notifications" @click.stop="toggleNotif">
          ...
          <Transition name="dropdown">
            <!-- FIX: NO @click.stop here — it was preventing router-link navigation -->
            <div v-if="showNotif" class="nav__dropdown nav__dropdown--notif" @click.stop>
              <div class="nav__dd-head">
                ...
                <button v-if="unreadNotifCount > 0" @click="markAllRead" class="nav__dd-mark">Mark all read</button>
              </div>
              ...
              <div class="nav__dd-footer">
                <router-link to="/notifications" @click="closeNotif" class="nav__dd-view-all">
        ...
        <!-- User menu -->
        <!-- FIX: ref + mousedown handler, no @click.stop on the dropdown -->
        <div class="nav__user-btn" ref="menuRef" @click.stop="toggleMenu">
          ...
          <Transition name="dropdown">
            <!-- FIX: removed @click.stop — it was blocking router-link clicks inside -->
            <div v-if="showMenu" class="nav__dropdown nav__dropdown--user">
              ...
              <router-link class="nav__dd-item" to="/profile"      @click="closeMenu">
              ...
              <router-link class="nav__dd-item" to="/orders"       @click="closeMenu">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2"/><rect x="9" y="3" width="6" height="4" rx="2"/><path d="M9 12h6M9 16h4"/></svg>
                My Orders
              </router-link>
              <router-link class="nav__dd-item" to="/sales"        @click="closeMenu">
                <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 3h18v4H3z"/><path d="M5 7v13h14V7"/><path d="M9 12h6"/></svg>
                My Sales
              </router-link>
              <router-link class="nav__dd-item" to="/dashboard"    @click="closeMenu">
              ...
              <router-link class="nav__dd-item" to="/messages"     @click="closeMenu">
              ...
              <router-link class="nav__dd-item" to="/add-product"  @click="closeMenu">
              ...
              <router-link v-if="auth.user?.is_admin" class="nav__dd-item" to="/admin" @click="closeMenu">
...
// Template refs for the two dropdown wrappers
...
// ── Watch profile pic ──────────────────────────────────────────────────────
watch(() => auth.user?.profile_pic, (val) => { profilePic.value = val || null }, { immediate: true })
...
const fetchCart = async () => {
  if (!auth.user) return
  try {
    const r = await api.getCart(auth.user.user_id)
    cartCount.value = Array.isArray(r.data) ? r.data.length : 0
  } catch {}
}
...
const fetchUnread = async () => {
  if (!auth.user) return
  try {
    const r = await api.getUnreadCount(auth.user.user_id)
...
const fetchNotifs = async (loader = false) => {
  ...
  try {
    const r = await api.getNotifications(auth.user.user_id)
...
const handleNotifClick = (notif) => {
  ...
  if (msg.includes('💬') || msg.toLowerCase().includes('message')) {
    router.push('/messages')
  } else if (msg.toLowerCase().includes('order') || msg.includes('🎉') || msg.includes('✅')) {
    router.push('/orders')
...
const toggleNotif = (e) => {
  ...
  if (showNotif.value) {
    ...
    fetchNotifs(!notifications.value.length)
  }
}
...
// ── Click outside — mousedown so it doesn't race with router-link clicks ──
// FIX: The old v-click-outside used 'click' events with @click.stop on the
// dropdown, which prevented the router-link's own click from ever firing.
// Using 'mousedown' (which fires BEFORE 'click') lets us close the dropdown
// while the subsequent 'click' event still reaches the router-link normally.
const onDocMousedown = (e) => {
  if (showMenu.value  && menuRef.value  && !menuRef.value.contains(e.target))  closeMenu()
  if (showNotif.value && notifRef.value && !notifRef.value.contains(e.target)) closeNotif()
}
...
onMounted(async () => {
  ...
  window.addEventListener('notif-cleared', onNotifCleared)
...
onUnmounted(() => {
  ...
  window.removeEventListener('notif-cleared', onNotifCleared)
  document.removeEventListener('mousedown', onDocMousedown)
})
</script>

<style scoped>
.nav { background:#fff; border-bottom:1px solid #e5eaf2; height:62px; position:sticky; top:0; z-index:999; box-shadow:0 1px 6px rgba(0,51,102,0.06); }
.nav__inner { max-width:1280px; margin:0 auto; height:100%; padding:0 24px; display:flex; align-items:center; gap:24px; }
...
.nav__logo-name { font-size:1.05rem; font-weight:800; color:#003366; letter-spacing:-0.5px; }
.nav__logo-accent { color:#c09b00; }

.nav__links { display:flex; gap:2px; flex:1; }
.nav__link { font-size:0.875rem; font-weight:500; color:#555; text-decoration:none; padding:6px 12px; border-radius:7px; transition:0.15s; }
.nav__link:hover, .nav__link.router-link-active { color:#003366; background:#eef3ff; font-weight:600; }

.nav__actions { display:flex; align-items:center; gap:2px; margin-left:auto; }
.nav__icon-btn { position:relative; width:38px; height:38px; border-radius:9px; display:flex; align-items:center; justify-content:center; color:#555; cursor:pointer; transition:0.15s; text-decoration:none; border:none; background:none; }
.nav__icon-btn:hover, .nav__icon-btn.router-link-active { color:#003366; background:#eef3ff; }

.nav__badge { position:absolute; top:2px; right:2px; background:#003366; color:#FFD700; font-size:9px; font-weight:800; min-width:15px; height:15px; border-radius:8px; display:flex; align-items:center; justify-content:center; padding:0 3px; border:1.5px solid #fff; pointer-events:none; }
.nav__notif-wrap { position:relative; }

.nav__user-btn { display:flex; align-items:center; gap:7px; cursor:pointer; padding:4px 10px 4px 4px; border-radius:9px; transition:0.15s; position:relative; border:1px solid transparent; margin-left:4px; user-select:none; }
.nav__user-btn:hover { background:#eef3ff; border-color:#dde8f5; }
.nav__avatar { width:30px; height:30px; border-radius:50%; background:#003366; overflow:hidden; display:flex; align-items:center; justify-content:center; flex-shrink:0; border:2px solid #FFD700; }
...
.nav__dropdown { position:absolute; top:calc(100% + 10px); right:0; background:#fff; border:1px solid #e4eaf2; border-radius:14px; box-shadow:0 12px 40px rgba(0,51,102,0.12); z-index:1000; overflow:hidden; min-width:220px; }
...
.nav__dd-head { display:flex; justify-content:space-between; align-items:center; padding:12px 16px 10px; border-bottom:1px solid #f0f0f0; position:sticky; top:0; background:#fff; z-index:1; }
...
.nav__dd-av { width:40px; height:40px; border-radius:50%; background:#003366; color:#FFD700; overflow:hidden; display:flex; align-items:center; justify-content:center; font-size:13px; font-weight:900; flex-shrink:0; border:2px solid #FFD700; }
...
.nav__dd-item { display:flex; align-items:center; gap:10px; padding:10px 16px; font-size:0.875rem; color:#444; text-decoration:none; background:none; border:none; width:100%; text-align:left; cursor:pointer; transition:0.12s; font-family:inherit; }
...
.nav__notif-item { display:flex; align-items:flex-start; gap:10px; padding:10px 16px; border-bottom:1px solid #f5f5f5; transition:background 0.12s; cursor:pointer; }
.nav__notif-item:last-of-type { border-bottom:none; }
...
.nav__dd-footer { padding:8px 16px; border-top:1px solid #f0f0f0; text-align:center; }
.nav__dd-view-all { font-size:0.78rem; font-weight:600; color:#003366; text-decoration:none; }
.nav__dd-view-all:hover { text-decoration:underline; }

.nav__auth-btns { margin-left:auto; display:flex; gap:8px; align-items:center; }
.nav__signin-btn { background:#003366; color:#fff; padding:8px 20px; border-radius:8px; font-weight:700; font-size:0.875rem; text-decoration:none; transition:0.15s; }
