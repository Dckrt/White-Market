    ...
    <div class="msg-layout" :class="{ 'msg-layout--open': activeThread }">
      ...
      <aside class="msg-sidebar" :class="{ 'msg-sidebar--hidden': activeThread }">
        ...
        <div v-else class="msg-thread-list">
          ...
          <div v-for="t in filteredThreads" :key="t.seller_id"
            class="thread-item"
            :class="{ 'thread-item--active': activeThread?.seller_id === t.seller_id, 'thread-item--unread': t.unread }"
            @click="openThread(t)">
            ...
            <div class="thread-body">
              ...
              <p class="thread-preview">{{ t.last_message || 'Start a conversation' }}</p>
            </div>
      ...
      <main v-if="activeThread" class="msg-chat">
        <div class="msg-chat__head">
          <button class="msg-back" @click="activeThread = null">
          ...
          <!-- Send message button in header for quick access -->
          <div style="margin-left:auto">
            <span class="msg-chat__hint">Press Enter to send</span>
          </div>
        </div>
        ...
        <div class="msg-list" ref="msgListRef">
          ...
          <template v-else>
            <div v-if="!messages.length" class="msg-empty-chat">
              <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#ccc" stroke-width="1.2"><path d="M21 15a2 2 0 01-2 2H7l-4 4V5a2 2 0 012-2h14a2 2 0 012 2z"/></svg>
              <p>No messages yet — say hello!</p>
            </div>
            <template v-for="(msg, i) in messages" :key="i">
              <div v-if="showDateSep(i)" class="date-sep"><span>{{ dateLabel(msg.sent_at) }}</span></div>
...
const fetchThreads = async (silent = false) => {
  ...
  try {
    ...
    // Don't mark active thread as unread
...
const send = async () => {
  ...
  try {
    ...
    // If thread doesn't exist in list yet, add it
    if (!t) {
      threads.value.unshift({
        seller_id:    activeThread.value.seller_id,
...
const pollMessages = async () => {
  ...
  try {
    ...
    if (fresh.length > messages.value.length) {
      ...
      await api.markMessagesRead({ reader_id: Number(user.user_id), sender_id: Number(activeThread.value.seller_id) }).catch(() => {})
    }
  } catch {}
}
...
onMounted(async () => {
  ...
  // Auto-open thread if seller_id in query
  const sid = route.query.seller_id
  if (sid) {
    const sidN = Number(sid)
    let t = threads.value.find(x => Number(x.seller_id) === sidN)
    if (!t) {
      t = { seller_id: sidN, seller_name: route.query.seller_name || 'Seller', last_message: '', last_time: '', unread: false }
...
/* Sidebar */
.msg-sidebar { border-right:1px solid #e8edf4; display:flex; flex-direction:column; background:#fafcff; }
.msg-sidebar__head { padding:18px 16px 12px; border-bottom:1px solid #eef2f8; display:flex; align-items:center; gap:10px; }
...
.msg-sidebar__search { padding:10px 12px; border-bottom:1px solid #eef2f8; display:flex; align-items:center; gap:8px; }
...
.msg-chat__head { display:flex; align-items:center; gap:10px; padding:14px 16px; border-bottom:1px solid #eef2f8; }
.msg-back { display:none; background:none; border:none; color:#003366; cursor:pointer; padding:4px; border-radius:7px; }
...
.self-warn { display:flex; align-items:center; gap:8px; background:#fffbeb; color:#92400e; font-size:0.82rem; font-weight:600; padding:10px 16px; border-bottom:1px solid #fde68a; }
...
.bubble { max-width:65%; padding:10px 14px 8px; border-radius:18px; font-size:0.875rem; line-height:1.5; word-break:break-word; position:relative; }
...
.msg-compose { display:flex; align-items:center; gap:10px; padding:12px 16px; border-top:1px solid #eef2f8; background:#fff; }
