    ...
    <div class="container">
      ...
      <div class="checkout-layout">
        ...
        <div class="checkout-form panel">
          ...
          <button class="btn-request" @click="placeRequest" :disabled="sending">
        ...
        <div class="checkout-summary panel">
          ...
          <div v-if="cart.length === 0" class="empty-note">No items in cart.</div>
  ...
  methods: {
    async placeRequest() {
      ...
      try {
        ...
        alert("Could not send request. Is Flask running?");
      } finally {
        this.sending = false;
      }
    }
...
.checkout-form h3, .checkout-summary h3 { font-size: 16px; margin: 0 0 16px; }
...
.pay-option input[type="radio"] { width: auto; margin: 0; padding: 0; accent-color: #03120E; }
.pay-option.active { border-color: #03120E; background: #f5f9f8; }
.btn-request {
  width: 100%;
  margin-top: 24px;
  padding: 14px;
...
.btn-request:hover { background: #0a2a23; }
.btn-request:disabled { background: #aaa; cursor: not-allowed; }
.summary-item { display: flex; align-items: center; gap: 12px; margin-bottom: 14px; }
.summary-item img { width: 56px; height: 56px; object-fit: cover; border-radius: 8px; background: #f0f0f0; }
.item-name { font-size: 14px; font-weight: 500; margin: 0 0 4px; color: #222; }
.item-price { font-size: 14px; font-weight: 700; color: #e60023; margin: 0; }
.summary-divider { border-top: 1px solid #eee; margin: 16px 0; }
.summary-total { display: flex; justify-content: space-between; font-size: 16px; font-weight: 700; color: #111; }
.empty-note { color: #aaa; font-size: 14px; }
</style>