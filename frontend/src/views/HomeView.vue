  ...
  <div class="wm-home">
    ...
    <section class="wm-hero">
      <div class="wm-hero__content">
        ...
        <div class="wm-hero__search">
          ...
          <div v-if="suggestions.length && heroSearch.length > 1" class="wm-hero__suggestions">
            <div v-for="s in suggestions.slice(0, 5)" :key="s.id" class="wm-hero__suggestion" @click="goToProduct(s.id)">
      ...
      <div class="wm-hero__stats">
        <div class="wm-stat">
          <span class="wm-stat__num">{{ liveStats.products !== null ? liveStats.products : '—' }}</span>
          <span class="wm-stat__lbl">Listings</span>
        </div>
        <div class="wm-stat-div"></div>
        <div class="wm-stat">
          <span class="wm-stat__num">{{ liveStats.users !== null ? liveStats.users : '—' }}</span>
          <span class="wm-stat__lbl">Students</span>
        </div>
        <div class="wm-stat-div"></div>
        <div class="wm-stat">
          <span class="wm-stat__num">{{ liveStats.colleges }}</span>
    ...
    <!-- ── POPULAR TAGS ── -->
    <section class="wm-section wm-section--tags" v-if="popularTags.length">
      <div class="wm-section__head">
    ...
    <section class="wm-section">
      ...
      <div v-else class="wm-empty">
        <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="#ccc" stroke-width="1.2" stroke-linecap="round"><path d="M6 2L3 6v14a2 2 0 002 2h14a2 2 0 002-2V6l-3-4z"/><line x1="3" y1="6" x2="21" y2="6"/><path d="M16 10a4 4 0 01-8 0"/></svg>
    ...
    <section class="wm-cta">
      <div class="wm-cta__icon">
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="#FFD700" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"><polygon points="13 2 3 14 12 14 11 22 21 10 12 10 13 2"/></svg>
      </div>
...
const liveStats   = ref({ products: null, users: null, colleges: 6 })
let debounceTimer = null
let statsInterval = null

// ── Category SVG icons ─────────────────────────────────────────────────────
const mkSvg = (paths, extra = []) => ({
  render: () => h('svg', { width:22, height:22, viewBox:'0 0 24 24', fill:'none', stroke:'currentColor', 'stroke-width':'1.8', 'stroke-linecap':'round', 'stroke-linejoin':'round' },
    [...paths.map(d => h('path', { d })), ...extra])
})

const IconBook    = mkSvg(['M4 19.5A2.5 2.5 0 016.5 17H20','M6.5 2H20v20H6.5A2.5 2.5 0 014 19.5v-15A2.5 2.5 0 016.5 2z'])
const IconLaptop  = { render: () => h('svg', {width:22,height:22,viewBox:'0 0 24 24',fill:'none',stroke:'currentColor','stroke-width':'1.8','stroke-linecap':'round'}, [h('rect',{x:2,y:3,width:20,height:14,rx:2}),h('line',{x1:8,y1:21,x2:16,y2:21}),h('line',{x1:12,y1:17,x2:12,y2:21})]) }
const IconHome    = mkSvg(['M3 9l9-7 9 7v11a2 2 0 01-2 2H5a2 2 0 01-2-2z','M9 22V12h6v10'])
const IconShirt   = mkSvg(['M20.38 3.46L16 2a4 4 0 01-8 0L3.62 3.46a2 2 0 00-1.34 2.23l.58 3.57a1 1 0 00.99.84H6v10c0 1.1.9 2 2 2h8a2 2 0 002-2V10h2.15a1 1 0 00.99-.84l.58-3.57a2 2 0 00-1.34-2.23z'])
const IconPencil  = mkSvg(['M12 20h9','M16.5 3.5a2.121 2.121 0 013 3L7 19l-4 1 1-4L16.5 3.5z'])
const IconCoffee  = { render: () => h('svg', {width:22,height:22,viewBox:'0 0 24 24',fill:'none',stroke:'currentColor','stroke-width':'1.8','stroke-linecap':'round'}, [h('path',{d:'M18 8h1a4 4 0 010 8h-1'}),h('path',{d:'M2 8h16v9a4 4 0 01-4 4H6a4 4 0 01-4-4V8z'}),h('line',{x1:6,y1:1,x2:6,y2:4}),h('line',{x1:10,y1:1,x2:10,y2:4}),h('line',{x1:14,y1:1,x2:14,y2:4})]) }
const IconTool    = mkSvg(['M14.7 6.3a1 1 0 000 1.4l1.6 1.6a1 1 0 001.4 0l3.77-3.77a6 6 0 01-7.94 7.94l-6.91 6.91a2.12 2.12 0 01-3-3l6.91-6.91a6 6 0 017.94-7.94l-3.76 3.76z'])
...
const goSearch    = () => { suggestions.value = []; if (heroSearch.value.trim()) router.push({ path: '/products', query: { search: heroSearch.value.trim() } }); else router.push('/products') }
...
const fetchStats = async () => {
  try {
    ...
    liveStats.value = { ...liveStats.value, ...r.data }
  } catch {}
}

onMounted(async () => {
  // Load stats immediately then poll every 10s
  await fetchStats()
...
/* Search */
.wm-hero__search { max-width:600px; margin:0 auto 1.75rem; position:relative; z-index:10; }
...
.wm-hero__suggestions { position:absolute; top:calc(100% + 6px); left:0; right:0; background:#fff; border-radius:12px; box-shadow:0 8px 32px rgba(0,0,0,0.15); overflow:hidden; z-index:100; text-align:left; border:1px solid #e8edf4; }
...
.wm-stat__num { font-size:1.8rem; font-weight:800; color:#FFD700; line-height:1; transition:all 0.3s; }
.wm-stat__lbl { font-size:0.75rem; color:rgba(255,255,255,0.6); margin-top:4px; }
