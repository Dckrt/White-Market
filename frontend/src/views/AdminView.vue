    ...
    <div class="adm-dash">
      ...
      <main class="adm-main">
        ...
        <section v-if="activeTab==='overview'" class="adm-section">
          ...
          <h2 class="adm-subtitle" style="margin-top:24px;margin-bottom:12px">Recent Listings</h2>
          <div class="adm-list">
...
const switchTab = (k) => {
  ...
  if (k==='orders'   && !allOrders.value.length)  loadOrdersTab()
}
...
const loadOrdersTab= async () => { loadingOrders.value=true; try { allOrders.value = (await api.adminOrders()).data   } catch { showToast('Failed','error') } finally { loadingOrders.value=false } }
...
const saveProd = async () => {
  if (!prodForm.value.title || !prodForm.value.price || !prodForm.value.category) {
    showToast('Title, price and category required','error'); return
  }
  if (!editingProd.value && !prodForm.value.seller_id) {
    showToast('Seller ID required','error'); return
  }
